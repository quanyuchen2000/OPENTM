...
void runbatch() {
	...
	while (std::getline(testfile, line)) {
		...
		cfg::HomoConfig config;
		config.init();
		config.heatRatio[0] = 400;
		config.heatRatio[1] = 0.17;
		config.testname = line;
		{
			for (int i = 0; i < 6; i++) {
				ss >> word;
				config.target_tensor[i] = std::stof(word);
			}
			ss >> word;
			// random, randcenter, noise, manual, interp, rep_randcenter, P, G, D, IWP, example
			if (word == "random") {
				config.winit = cfg::InitWay::random;
			}
			else if (word == "randcenter") {
				config.winit = cfg::InitWay::randcenter;
			}
			else if (word == "noise") {
				config.winit = cfg::InitWay::noise;
			}
			else if (word == "manual") {
				config.winit = cfg::InitWay::manual;
			}
			else if (word == "interp") {
				config.winit = cfg::InitWay::interp;
			}
			else if (word == "rep_randcenter") {
				config.winit = cfg::InitWay::rep_randcenter;
			}
			else if (word == "P") {
				config.winit = cfg::InitWay::P;
			}
			else if (word == "D") {
				config.winit = cfg::InitWay::D;
			}
			else if (word == "G") {
				config.winit = cfg::InitWay::G;
			}
			else if (word == "IWP") {
				config.winit = cfg::InitWay::IWP;
			}
			else if (word == "example") {
				config.winit = cfg::InitWay::example;
			}
			ss >> word;
			if (word == "NONE") {
				config.sym = cfg::Symmetry::NONE;
			}
			else if (word == "rotate2") {
				config.sym = cfg::Symmetry::rotate2;
			}
			ss >> word;
			config.reso[0] = std::stoi(word);
			config.reso[1] = std::stoi(word);
			config.reso[2] = std::stoi(word);
			ss >> word;
			if (word == "mma") {
				config.model = cfg::Model::mma;
			}
			else if (word == "oc") {
				config.model = cfg::Model::oc;
			}
			else if (word == "moo") {
				config.model = cfg::Model::moo;
			}
			runCustom(config);
		}
	}
...
int main()
{
	...
	//	exit(-1);
	//}
	runInstance(256, { 400, 0.17 }, {60,60,60,0,0,0}, cfg::InitWay::IWP, cfg::Model::oc);
	// runbatch();
	return 0;
}
