...
#pragma unroll
	...
	Tout norm(T* in_datax, size_t n, Tout* sum_dst = nullptr) {
	...
	template<typename T, typename Tout>
	T dot(T* pdatax, T* qdatax, Tout* odata, size_t n, Tout* sum_dst = nullptr) {
		// todo : use flattenned v3 rather than distinct pointer via index transform
		auto gen = [=] __device__(int tid) {
			T x1 = pdatax[tid];
			T x2 = qdatax[tid];
	...
	__host__ __device__ inline float tanproj(float val, float beta, float tau = 0.5f) {
