...
template<typename T>
__global__ void heatMatrix_kernel_opt_host(
	int nv,
	devArray_t<devArray_t<float*, 1>, 3> ucharlist,
	T* rholist, VertexFlags* vflags, CellFlags* eflags,
	float* elementCompliance, int pitchT
...
template <typename T, int BlockSize = 256>
__global__ void fillTotalVertices_kernel_host_H(
	int nv, VertexFlags* vflags,
	devArray_t<devArray_t<T*, 1>, 3> uchar,
	devArray_t<devArray_t<float*, 1>, 3> dst)
{
	...
	GridVertexIndex indexer(ereso[0], ereso[1], ereso[2]);
...
void homo::Homogenization_H::heatMatrix(double C[3][3]) {
	...
	for (int i = 0; i < 3; i++) {
		...
		if (!grid->use_host_memory) {
			...
			grid->u_h = std::move(grid->uchar[i]);
		...
		if (!grid->use_host_memory) {
			...
			grid->uchar[i] = std::move(grid->u_h);
		}
	}
	...
	grid->useGrid_g();
	if (grid->use_host_memory) {
		// go strait to heatMatrix_kernel_opt
		...
		for (int blockid = 0; blockid < block_num; blockid++) {
			...
			fillTotalVertices_kernel_host_H << <grid_size, block_size >> > (nv, vflags, ucharlist, dst);
			...
			//_TIC("ematopt")
			heatMatrix_kernel_opt_host << <grid_size, block_size >> > (nv, dst,
				grid->rho_g, vflags, eflags,
			...
			for (int i = 0; i < 3; i++) {
				for (int j = i; j < 3; j++) {
					C[j][i] += parallel_sum(buffer.template data<float>() + counter * pitchT, pitchT) / vol;
					counter++;
				}
			}
...
template <typename T, int BlockSize = 256>
__global__ void Sensitivity_kernel_opt_host_H(
...
#pragma unroll
	...
	if (warpId < 1 && !is_ghost && elementId != -1) {
		...
		if (!lexiOrder) {
			...
			int lexid = p.x + (p.y + p.z * MIN_TRANSFER) * MIN_TRANSFER;
			sens[lexid] = s;
		}
	}
}
