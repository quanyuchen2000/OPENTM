...
void homo::Grid_H::enforce_vertex_boundary(std::vector<VT>& v) {
	// seam align to right 
	// update padding
	nvtxRangePushA("vertex_boundary_phase");
	...
	int block_num = block_numx * block_numy * block_numz * 9;
	...
	for (unsigned t = 0; t < num_threads; ++t) {
		workers.emplace_back([&, block_num] {
			while (true) {
				const int vid = next_bid.fetch_add(1, std::memory_order_relaxed);
				if (vid >= block_num) break;
				const int bid = vid / 9;
...
void homo::Grid_H::enforce_vertex_boundary_block(std::vector<VT>& v, int blockid, std::vector<std::thread> &workers) {
	// seam align to right 
	// update padding
	bid = blockid;
	if (!workers.empty()) {
