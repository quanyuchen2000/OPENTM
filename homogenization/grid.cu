...
__global__ void restrict_residual_kernel_host_H(
	...
	int posvec[3] = { pos.x, pos.y, pos.z };
	if (!fiction && !vflag.is_period_padding()) {
		int upper_vertex[3][2] = { {-1,-1,},{-1,-1},{-1,-1} };
#pragma unroll
		for (int i = 0; i < 3; i++) {
			upper_vertex[i][0] = posvec[i] / coarseRatio[i] * coarseRatio[i];
			if (posvec[i] % coarseRatio[i] != 0) {
				upper_vertex[i][1] = (posvec[i] / coarseRatio[i] + 1) * coarseRatio[i];
			}
		}
		for (int i = 0; i < 8; i++) {
			int iddx = i % 2;
			int iddy = i / 2 % 2;
			int iddz = i / 4;
			if (upper_vertex[0][iddx] != -1 && upper_vertex[1][iddy] != -1 && upper_vertex[2][iddz] != -1) {
				int off_pos[3] = { upper_vertex[0][iddx] , upper_vertex[1][iddy] , upper_vertex[2][iddz] };
				float w = (coarseRatio[0] - abs(off_pos[0] - posvec[0])) * (coarseRatio[1] - abs(off_pos[1] - posvec[1])) * (coarseRatio[2] - abs(off_pos[2] - posvec[2])) / pr;
